...
")}"
  ...
  def send_to_client(player_id, message)
    ...
    begin
      ...
      # Use WebSocket send instead of custom frame creation
      client[:connection].websocket.send(json_message)
...
# Start the server using Faye-WebSocket (like your working test server)
if __FILE__ == $0
  EventMachine.run do
    ...
    App = lambda do |env|
      if Faye::WebSocket.websocket?(env)
        ...
        # Create a connection wrapper for compatibility with your existing game server
        ...
        # Override send_data method to use WebSocket send
        def connection.send_data(data)
          self.websocket.send(data)
        end
        
        # Override close_connection method
        def connection.close_connection
          self.websocket.close
        end
        
        ws.on :open do |event|
          puts "✅ WebSocket connection opened successfully"
          game_server.handle_connection(connection)
        end

        ws.on :message do |event|
          puts "📨 Received message: #{event.data[0..100]}..."
          begin
            data = JSON.parse(event.data)
            game_server.handle_message(connection.player_id, data)
          rescue JSON::ParserError => e
            puts "❌ Invalid JSON: #{e.message}"
            ws.send(JSON.generate({ error: 'Invalid JSON format' }))
          end
        end
        ...
        ws.on :close do |event|
          ...
          if connection.player_id
            game_server.handle_client_disconnect(connection.player_id)
          end
        end
    ...
    # Use Thin server like your working test server
