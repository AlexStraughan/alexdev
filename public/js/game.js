...
class Game {
    constructor() {
        ...
        this.generatorData = [];
        this.upgradeData = []; // Will be populated from API
    ...
    async loadGameData() {
        try {
            ...
            // Load upgrades from API and transform to expected format
            const upgradesResponse = await fetch('/api/upgrades');
            const upgradesData = await upgradesResponse.json();
            
            // Transform API upgrade format to frontend format
            this.upgradeData = upgradesData.upgrades.map(upgrade => {
                const transformed = {
                    id: upgrade.id,
                    name: upgrade.name,
                    ...
                    targetGenerator: upgrade.affects,
                    effect: upgrade.affects === 'all' ? 'globalMultiplier' : 'generatorMultiplier'
                };
                
                // Add category based on the generator it affects
                if (upgrade.affects === 'all') {
                    transformed.category = 'global';
                } else {
                    transformed.category = upgrade.affects;
                }
            ...
            console.log('🔄 Transformed upgrade sample:', this.upgradeData[0]);
